...
class NovelGridworldV0Env(gym.Env):
    ...
    def __init__(self, map_width=None, map_height=None, items_id=None, items_quantity=None, initial_inventory = None, no_fire = None, goal_env = None, is_final = False):
        ...
        self.items_quantity = {'tree': 5, 'rock': 2, 'fire': self.is_fire, 'crafting_table': 1, 'pogo_stick': 0, 'tent': 0, 'tent_area': 1}
    ...
    def assert_conditions(self):
        ...
        if self.goal_env == 3:
            total_craftable_pogo_sticks = min((self.items_quantity['rock'] + self.initial_inventory['rock']), ((self.initial_inventory['tree'] + self.items_quantity['tree']) // 2 ))
            ...
            if total_craftable_pogo_sticks > 0 and self.initial_inventory['pogo_stick'] <2 and self.initial_inventory['tent'] <= 0:
    ...
    def step(self, action):
        ...
        # Break
            ...
            if self.block_in_front_str == 'tree' or self.block_in_front_str == 'rock':
                ...
                if (self.block_in_front_str == 'tree' and self.inventory_items_quantity['tree'] <= 4) or (self.block_in_front_str == 'rock' and self.inventory_items_quantity['rock'] <= 1):
        ...
        if self.goal_env == 2: # If the goal is crafting
            if (self.inventory_items_quantity['tree'] >= 2 and self.inventory_items_quantity['tree'] < 3 and self.inventory_items_quantity['rock'] > 0) or \
